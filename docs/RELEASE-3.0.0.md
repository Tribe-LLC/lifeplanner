...
- **Both migration chains are covered, and the numbers in this bullet used to be wrong.** 2.3 did not ship at schema v21. It shipped from `003b892`, where the highest migration file was `17.sqm`, and SQLDelight derives the schema version as the highest migration plus one, so a 2.3 device sits at **18**. The current generated `Schema.version` is **43**, not the 42 declared in `build.gradle.kts` (same derivation). Upgraders therefore run **18→43**. `DatabaseMigrationsTest` runs the hand-written Android chain and `SqmMigrationTest` runs the `.sqm` chain iOS uses, both against a 2.3-shaped database, and both assert the result has every one of the 484 columns `Schema.create` gives a fresh install. A third test asserts the two chains land on the same schema, since they are separate implementations free to drift. Rows are asserted to survive, including `HealthMetricEntity`. Still JVM schema tests, not a 2.3-install-upgraded-on-device run.
- **DB schema v42** (v36 when this doc was written, v39 at the 2026-08-04 pass; the Learn hub content cache, journal-detected decisions, habit `completionSource`, then the Wheel of Life tables landed since). Runtime Android migrations exist through `migrateToVersion42` in `DatabaseMigrations.kt` and every one of them is wired into the `runAndroidMigrations` chain called from `DatabaseDriverFactory.onOpen()` (re-verified 2026-08-20); `.sqm` files through 42 present for iOS. There is **no** compile-time verification of them: `verifyMigrations` is off and no schema snapshots are committed, so nothing checked the `.sqm` files against the schema until `SqmMigrationTest` was written. See the bullet above for the real version numbers.
